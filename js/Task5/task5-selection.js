...
function routeTask5Automatically() {
  ...
  if (completedModes.includes("overload") && completedModes.includes("zen")) {
    console.log("Both Task 5 modes already completed. Redirecting to survey.");
    setTimeout(() => {
  ...
  const firstMode = getFirstModeForTask5(group);
  ...
  if (!nextMode) {
    console.warn("Could not determine next Task 5 mode. Redirecting to survey.");
    setTimeout(() => {
      window.location.href = "task5-survey.html";
    }, 500);
    return;
  ...
  console.log("Task 5 auto-routing to:", nextMode);
...
function getFirstModeForTask5(group) {
  // Group 1 -> Task 5: A first -> overload first
  // Group 2 -> Task 5: B first -> zen first
  if (group === "group2") {
    return "zen";
  }
  return "overload";
...
function selectMode(mode) {
  console.log(`Task 5 auto-selected mode: ${mode}`);
